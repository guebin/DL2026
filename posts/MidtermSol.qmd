---
title: "**중간고사 수정 & 해설 (2026.05.04)**"
author: "최규빈"
date: "05/05/2026"
draft: false
---

{{<video https://youtu.be/playlist?list=PLQqh36zP38-ycHUuPXaF1e2zVsZ3r1z6d&si=Vl5QsYh6exU7lG8p>}}
...
> **참고**: 이하 [(3)~(7)]{style="color:red"} 에서 "적합이 잘 된다" 는 모든 $i = 1, 2, 3, 4$ 에 대해 $\hat y_i \approx y_i$ 임을 의미한다.
...
`(4)` 자료 1 의 코드에서 다른 부분은 그대로 두고 학습률만 $\alpha = 10^{-6}$ 으로 매우 작게 바꾸어 다시 실행하였다. [15 회]{style="color:red"} 반복 후 `p.data` 의 값에 대한 진술로 가장 적절한 것은?

① 매 반복의 갱신폭이 너무 작아 [15 회]{style="color:red"} 반복으로는 거의 움직이지 못하므로 `p.data` 는 초기값 $0.5$ 근처에 머무름\
② 학습률을 줄이면 더 정확하게 진짜 모수 $p^\ast = 0.3$ 으로 수렴함\
③ 학습률의 크기에 무관하게 15 회 반복 안에 항상 $0.2926$ 에 정확히 도달함\
...
`(1)` 그림에서 최소점 $x = 2$ 의 [근방으로]{style="color:red"} 도달하는 데^[[근방으로 도달의 의미: 임의의 $\varepsilon > 0$ 에 대해 어떤 자연수 $N$ 이 존재하여 $n \geq N$ 일 때 $|x_n - 2| < \varepsilon$ 이 성립함 (즉, 수열 $\{x_n\}$ 이 $2$ 로 수렴함).]{style="color:red"}] **가장 많은 반복** 이 필요한 학습률은?
...
`(7)` 다음 두 네트워크 중 [$0 \sim 1$ 사이에 있는]{style="color:red"} 더 복잡한 곡선을 적합할 수 있는 것은?
...
[(원본 문제에 누락된 `torch.nn.Sigmoid()` 를 마지막 층에 추가)]{style="color:red"}

① (가)\
② (나). 히든 노드가 더 많아 [시그모이드를 취하기 전]{style="color:red"} 더 많이 꺾인 직선을 표현할 수 있음\
③ 둘은 동일한 표현력\
④ 둘 다 직선만 표현 가능

> 정답: ② ([문제 오류로 ①, ②, ③, ④ 모두 정답으로 인정]{style="color:red"})

`(8)` [$0 \sim 1$ 사이에 있는]{style="color:red"} 어떤 곡선 데이터를 위 `net_a`, `net_b` 로 적합한다고 하자. 데이터 수 $n$ 을 점점 키울 때 다음 중 옳은 것은? (학습률·epoch 충분히 적절히 가정)

① $n$ 이 커지면 어떠한 네트워크라도 참함수에 정확히 가까워진다\
② [$n$ 이 커진다고 해서 표현력이 자동으로 늘어나지 않음. 따라서 $n$ 이 커져도 `net_a`, `net_b` 모두 참함수에 가까워지지 않음]{style="color:red"}\
③ $n$ 이 커지면 네트워크가 꺾을 수 있는 횟수가 자동으로 증가한다\
④ [$n$ 의 크기는 적합 결과 $\hat{\bf Y}$ 와 무관]{style="color:red"}
...
(가) 시벤코 정리는 충분한 노드를 가진 1-은닉층 네트워크가 임의의 [보렐가측]{style="color:red"}함수를 원하는 정확도로 **표현** 할 수 있음을 보장한다.\
